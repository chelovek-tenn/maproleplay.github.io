...
// Vérifier si le paramètre marker est présent
if (urlParams.has('marker')) {
  // Obtenir les coordonnées du marker de l'URL
  const markerCoords = urlParams.get('marker').split(',');

  // Créer un nouveau marqueur avec les coordonnées récupérées
  const newMarker = L.marker([parseFloat(markerCoords[0]), parseFloat(markerCoords[1])], { icon: cardIcon1});
